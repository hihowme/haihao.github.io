...
<ol class="research-list">
  <li>
    ...
    <details>
      ...
      Popular music is inherently historical, with new songs exhibiting systematic similarity to past musical styles, and this process has become increasingly consequential as generative AI enables music creation at unprecedented scale. This paper proposes a novel data-driven framework for designing successful music with controlled similarity to past music styles. We first construct an Interpretable Music Attribute Representation (IMAR) by separating each track into stems, then extracting theory-grounded features for melody, harmony, rhythm, and timbre using deep learning models integrated with music-theoretic analysis. Using all Billboard Hot 100 (24,618) songs from 1958–2017, we cluster songs within rolling windows via clustering and then aggregate medoids using a forward linkage rule to obtain robust historical “styles.” Utilizing 29,331songs from 2018–2025, we then compute similarity measures between focal songs and these styles and show that successful contemporary music achieves optimal: melodies and rhythms that are close to past styles, fusion-oriented harmonies, and innovative timbres. Next, we train a state-of-the-art predictive model of Billboard entry based on audio-derived IMAR features and demonstrate that with only four of style-based similarity measures yields substantial gains in predictive accuracy. Finally, we operationalize these insights for generative AI: using a retrieval-augmented generation scheme that conditions on historical style medoids, we edit a current unpopular song to achieve the optimal distance to the past, which achieves the huge gain in predicted success compared with the current songs. Our framework offers one of the first end-to-end, implementable guidelines for using generative AI to solve the product design problem in music creation.
    </details>
  </li>
</ol>
</section>
